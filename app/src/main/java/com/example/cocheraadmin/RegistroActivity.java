...
public class RegistroActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    {
        FirebaseUser adm = FirebaseAuth.getInstance().getCurrentUser();
        if (adm!=null){
            adm.sendEmailVerification().addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()){
                        Toast.makeText(RegistroActivity.this,"Revise su correo electrónico para verificar", Toast.LENGTH_SHORT).show();
                        FirebaseAuth.getInstance().signOut();
                        Intent d = new Intent(RegistroActivity.this, LoginActivity.class);
                        startActivity(d);
                    }
                }
            });
    ...
    private void createAnewUser(String uid){
        admin adm = BuildNewuser();
        mDatabase.child(uid).setValue(adm);
    }
    private admin BuildNewuser(){
        return new admin(
                getnombre(),
                getapellido(),
                getcorreo(),
